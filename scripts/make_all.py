...
def run():
    ...
    # create puc
    df2objs(pd.read_csv('_files/puc.csv'), _raw_data_info, True)
    dt3 = datetime.now()
    print('PUC: {}'.format(dt3-dt2))

    # create phones
    df2objs(pd.read_csv('_files/phone.csv'), _raw_data_info, True)
    dt4 = datetime.now()
    print('Phones: {}'.format(dt4-dt3))

    # create emails
    df2objs(pd.read_csv('_files/e_mail.csv'), _raw_data_info, True)
    dt5 = datetime.now()
    print('Emails: {}'.format(dt5-dt4))

    # create people
    df2objs(pd.read_csv('_files/person.csv'), _raw_data_info, True)
    dt6 = datetime.now()
    print('People: {}'.format(dt6-dt5))

    # create people_phones
    df2objs(pd.read_csv('_files/person_phone.csv'), _raw_data_info, True)
    dt7 = datetime.now()
    ...
    df2objs(pd.read_csv('_files/person_e_mail.csv'), _raw_data_info, True)
    dt9 = datetime.now()
    print('People_Emails: {}'.format(dt9-dt8))

    # create estates
    df2objs(pd.read_csv('_files/estate.csv'), _raw_data_info, True)
    dt10 = datetime.now()
    print('Estates: {}'.format(dt10-dt9))

    # create estates_people
    df2objs(pd.read_csv('_files/estate_person.csv'), _raw_data_info, True)
    dt11 = datetime.now()
    print('Estates_People: {}'.format(dt11-dt10))

    # create estates_appraisals
    df2objs(pd.read_csv('_files/estate_appraisal.csv'), _raw_data_info, True)
    dt12 = datetime.now()
    print('Appraisals: {}'.format(dt12-dt11))

    # create realties
    df2objs(pd.read_csv('_files/realty.csv'), _raw_data_info, True)
    dt13 = datetime.now()
    print('Realties: {}'.format(dt13-dt12))

    # create realties_estates
    df2objs(pd.read_csv('_files/realty_estate.csv'), _raw_data_info, True)
    dt14 = datetime.now()
    print('Realties_Estates: {}'.format(dt14-dt13))

    # create accounts
    df2objs(pd.read_csv('_files/account.csv'), _raw_data_info, True)
    dt15 = datetime.now()
    print('Accounts: {}'.format(dt15-dt14))

    # create leases_realties
    df2objs(pd.read_csv('_files/lease_realty.csv'), _raw_data_info, True)
    dt16 = datetime.now()
    print('Leases_Realties: {}'.format(dt16-dt15))

    # create leases_realties_realties
    df2objs(pd.read_csv('_files/lease_realty_realty.csv'), _raw_data_info, True)
    dt17 = datetime.now()
    ...
    # create dates_values
    df2objs(pd.read_csv('_files/date_value.csv'), _raw_data_info, True)
    dt19 = datetime.now()
    print('Dates_Values: {}'.format(dt19-dt18))

    # create transaction_types
    df2objs(pd.read_csv('_files/transaction_type.csv'), _raw_data_info, True)
    dt20 = datetime.now()
    ...
    df2objs(pd.read_csv('_files/ledger_template.csv'), _raw_data_info, True)
    dt27 = datetime.now()
    print('Ledger_Template: {}'.format(dt27-dt26))

    # create charges templates
    df2objs(pd.read_csv('_files/charge_template.csv'), _raw_data_info, True)
    dt28 = datetime.now()
